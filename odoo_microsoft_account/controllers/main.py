...
class OAuthController(http.Controller):
    ...
    def microsoft_signin(self, **kw):
        ...
        try:
            ...
            print(e)
        # employee_id = request.env['hr.employee'].sudo().search([("work_email", "=", mail)])
        # if employee_id:
        #     pos_user_group = request.env.ref('point_of_sale.group_pos_user').id
        #     pos_manager_group = request.env.ref('point_of_sale.group_pos_manager').id
        #     user_id = request.env['res.users'].create({
        #         'name': displayName,
        #         'customer_account': 'none',
        #         'login': mail,
        #         'email': mail if mail else mail,
        #         'company_id': 1,
        #         # 'password': values['password'],
        #         'groups_id': [(6, 0, [request.env.ref('base.group_user').id,
        #                               pos_user_group if employee_id.employment_type == 'Cashier' else pos_manager_group
        #                               ])]
        #     })
